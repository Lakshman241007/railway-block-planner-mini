"""
TDMS Validator — validates raw TDMS train/traffic operational records.
...
class TDMSValidator:
    """Validates a single raw TDMS record."""

    def validate(self, record: Dict[str, str]) -> Tuple[bool, List[str]]:
        """
        Validate a raw TDMS record.
        ...
        self._validate_time(record, "expected_arrival", errors)
        self._validate_time(record, "expected_departure", errors)
        ...
        if not is_valid:
            ...
            logger.warning(
                "Validation failed for TDMS record '%s': %s",
                train_id,
                "; ".join(errors),
            )

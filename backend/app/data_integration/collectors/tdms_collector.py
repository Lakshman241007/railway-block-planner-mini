"""
TDMS Collector — reads raw train/traffic operational data from a CSV file.
...
-----
    ...
    collector = TDMSCollector(file_path=Path("data/raw/tdms/mock_tdms.csv"))
...
class TDMSCollector:
    """
    Collects raw TDMS operational records from a CSV file.

    Parameters
    ----------
    file_path : Path | str
        Path to the TDMS CSV file.
    """
    ...
    def collect(self) -> List[Dict[str, str]]:
        ...
        if not self.file_path.exists():
            raise FileNotFoundError(
                f"TDMS CSV file not found: {self.file_path.resolve()}"
            )

        logger.info("Reading TDMS data from %s", self.file_path.resolve())
        ...
        if not records:
            raise ValueError(
                f"TDMS CSV file is empty or contains no data rows: "
                f"{self.file_path.resolve()}"
            )

        logger.info("Collected %d raw TDMS records", len(records))
        return records
